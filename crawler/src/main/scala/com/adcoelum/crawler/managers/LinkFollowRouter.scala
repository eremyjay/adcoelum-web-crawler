...
import com.adcoelum.crawler.workers.LinkFollower
...
/**
  * Managing actor for performing routing for Link Follower
  */
class LinkFollowRouter extends Actor {
	...
	def setupRouter = {
		router = {
			...
			val routees = Vector.fill(Configuration.crawlerWorkerRoutees) {
				val dataPointProcessor = context.actorOf(Props[LinkFollower])
	...
	def receive = {
    	...
    	case Terminated(a) => {
      		...
      		val linkFollower = context.actorOf(Props[LinkFollower])
      		context watch linkFollower
      		router = router.addRoutee(linkFollower)
      	}

		// Check the activity of the router	and track number of messages
		case TriggerActivity() => {
			sender ! Activity("link_follower", messagesLastCheck, messages)
