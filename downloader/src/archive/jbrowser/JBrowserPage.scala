...
/**
  ...
  * @param instance
  */
class JBrowserPage(requestUrl: URL, responseUrl: URL, var page: WebElement, browser: JBrowserWebClient, instance: String) extends Page {
    // Xml content of the page
    val xml = page.getAttribute("outerHTML")
    ...
    def searchXPath(query: String): List[Element] = {
        ...
        Logger.trace(s"Perform XPath search with query:\n${query}\non page: ${requestUrl}", Logger.HYPER_VERBOSE_INTENSITY)
        val list: java.util.List[WebElement] = page.findElements(By.xpath(query))
        val result: ListBuffer[Element] = ListBuffer.empty

        // Create wrapper elements for each result
        list.asScala foreach { element =>
            if (element.isDisplayed)
                result += new JBrowserElement(this, element, browser, instance)
        }
    ...
    def executeScriptForPage(script: String): Page = {
        ...
        new JBrowserPage(responseUrl, responseUrl, page, browser, instance)
    }
