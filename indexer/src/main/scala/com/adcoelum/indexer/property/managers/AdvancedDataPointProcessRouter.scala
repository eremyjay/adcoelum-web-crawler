...
import com.adcoelum.indexer.property.workers.AdvancedDataPointProcessor
...
/**
  * Managing actor for performing routing for Advanced Data Point Processor
  */
class AdvancedDataPointProcessRouter extends Actor {
    ...
    def setupRouter = {
        router = {
            ...
            val routees = Vector.fill(Configuration.crawlerWorkerRoutees) {
                val dataPointProcessor = context.actorOf(Props[AdvancedDataPointProcessor])
    ...
    def receive = {
        ...
        case TriggerActivity() => {
            sender ! Activity("advanced_data_point_processor", messagesLastCheck, messages)
